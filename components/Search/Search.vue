<template>
  <v-row class="align-center">
    <v-icon class="primary pa-2 rounded-lg" color="white" size="19"> search</v-icon>
    ...
    <v-menu offset-y>
      ...
      <div>
        <v-card v-if="show_search" class="pa-3">
          ...
          <!-- no result -->
          <v-row
            v-if="product_categories.length == 0 && products.length == 0 && !loading"
            no-gutters
            class="flex-column"
          >
            <v-col class="mb-3 text-center text--secondary"> موردی یافت نشد </v-col>
          </v-row>
          <!-- no result end-->

          <!-- categorys -->
          <v-row v-if="product_categories.length != 0" no-gutters class="flex-column">
            <v-col
          ...
          <v-divider v-if="product_categories.length != 0" class="py-2"></v-divider>
