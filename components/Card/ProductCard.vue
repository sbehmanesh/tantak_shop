    ...
    <div>
        ...
        >
            ...
            style="position:absolute; width: 100%;bottom:0 ;">
                <div class="error whited--text font_12 py-1 px-2 mt-3">{{discount_price_percent}}%</div>
            </div>
            <!-- bottom img end -->
    ...
    computed:{
        ...
        price_text(){
            ...
            if(this.data.has_whole_sell == 1 && this.data.has_single_sell == 0){
                return "یک "+this.data.base_whole_sell_unit+" "+Number(this.price).toLocaleString()
        ...
        clacDiscount() {
            if (this.data.discounted_base_price && this.data.active_discount) {
                if (this.data.active_discount.type == "percent") {
                    this.final_discount = Number(this.data.active_discount.combination.price) * Number(this.data.active_discount.discount) / 100;
                }
            ...
            if (this.data.discounted_base_price && !this.data.active_discount) {
                this.final_discount = Number(this.price) - Number(this.data.discounted_base_price);
            }
            if (!this.data.discounted_base_price && !this.data.active_discount) {
