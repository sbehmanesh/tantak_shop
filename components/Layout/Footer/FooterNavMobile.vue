<template>
  ...
  <v-footer v-if="!$vuetify.breakpoint.mdAndUp" padless fixed class="elevation-3">
    <v-row
      no-gutters
      class="align-center justify-space-around background2 elevation-2"
      style="height: 56px"
    >
        ...
        <v-hover v-slot="{ hover }">
          <div class="d-flex flex-column align-center" style="min-width: 60px">
            ...
            <v-icon
              ...
              v-else
              size="40"
              :color="hover ? 'primary' : 'blackd'"
              >mdi-account-outline</v-icon
            >
            <v-scroll-y-transition>
              <div
                v-if="$route.path == '/profile' || hover"
                class="primary--text pa-0 text-center font_10"
              >
                پروفایل
              </div>
            </v-scroll-y-transition>
          ...
          <div class="d-flex flex-column align-center" style="min-width: 60px">
            ...
            >
              <v-icon
                size="36"
                aria-label="سبد خرید"
                :color="hover || $route.path == '/basket' ? 'primary' : 'blackd'"
                >mdi-cart-outline</v-icon
              >
            </v-badge>
            <v-scroll-y-transition>
              <div
                v-if="$route.path == '/basket' || hover"
                class="primary--text pa-0 text-center font_10"
              >
                سبد خرید
              </div>
